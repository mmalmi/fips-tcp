...
export var Flags;
(function (Flags) {
    Flags[Flags["Fin"] = 1] = "Fin";
    Flags[Flags["Syn"] = 2] = "Syn";
    Flags[Flags["Rst"] = 4] = "Rst";
    Flags[Flags["Psh"] = 8] = "Psh";
    Flags[Flags["Ack"] = 16] = "Ack";
    Flags[Flags["Ece"] = 64] = "Ece";
    Flags[Flags["Cwr"] = 128] = "Cwr";
})(Flags || (Flags = {}));
const SUPPORTED_FLAGS = Flags.Fin | Flags.Syn | Flags.Rst | Flags.Psh | Flags.Ack | Flags.Ece | Flags.Cwr;
export class FlagSet {
    bits;
    constructor(bits = 0) {
        this.bits = bits;
        if ((bits & ~SUPPORTED_FLAGS) !== 0)
            throw new Error(`unsupported TCP flags: ${bits}`);
    }
    has(flag) {
...
export var TcpOptionKind;
(function (TcpOptionKind) {
    TcpOptionKind["EndOfList"] = "end";
    TcpOptionKind["NoOperation"] = "nop";
    TcpOptionKind["MaxSegmentSize"] = "mss";
    TcpOptionKind["FipsVersion"] = "fips-version";
    TcpOptionKind["Unknown"] = "unknown";
})(TcpOptionKind || (TcpOptionKind = {}));
const checkU16 = (value, name) => {
    if (!Number.isInteger(value) || value < 0 || value > 0xffff) {
        throw new Error(`${name} must be a u16`);
    }
};
const checkU32 = (value, name) => {
...
export class Segment {
    ...
    constructor(init) {
        ...
        if (init.ack !== undefined)
            checkU32(init.ack, "acknowledgment number");
    ...
    supportsFipsVersion(expected) {
        return this.options.some((option) => option.kind === TcpOptionKind.FipsVersion &&
    ...
    encode() {
        if (this.srcPort === 0 || this.dstPort === 0)
            throw new Error("TCP/FIPS ports must be non-zero");
        ...
        if (headerLength > 60)
            throw new Error("TCP/FIPS header exceeds 60 bytes");
    ...
    static decode(bytes) {
        if (bytes.length < HEADER_LEN)
            throw new Error("truncated TCP/FIPS segment");
        ...
        if (view.getUint16(16) !== 0)
            throw new Error("TCP/FIPS checksum must be zero");
        if (view.getUint16(18) !== 0)
            throw new Error("TCP/FIPS urgent data is unsupported");
...
const cloneOption = (option) => option.kind === TcpOptionKind.Unknown ? { ...option, data: option.data.slice() } : { ...option };
